...
def cast_list(title)
  ...
  Actor.select(:id, :name).joins(:movies)
    .where("title = ?", title).order("ord asc")
end
...
def vanity_projects
  ...
  Movie.select("movies.id, title, name").joins(:actors)
...
def most_supportive
  ...
  Actor.select("actors.id, name, COUNT(movie_id) AS roles")
    .joins(:movies).where("ord != 1").group("actors.id")
    .order("COUNT(movie_id) desc").limit(2)
end
